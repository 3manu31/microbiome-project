...
import os
from microbiome_utils import get_top_microbes
...
top_microbes = get_top_microbes(merged, group_col, len(metadata.columns), top_n)
...
comparison_data = {}
metadata_columns_count = len(metadata.columns)
...
overall_mean = merged.iloc[:, :-metadata_columns_count].mean(axis=0).loc[all_top_microbes]
